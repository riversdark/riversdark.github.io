...
I took the _Solve It with Code_ course from answer.ai, and Advent of Code (AoC) was used there as the programming/problem solving exercise. So here I am, solving AoC for the first time.

I have no formal formation in computer science, and I've always considered my Python skills to be rather mediocre: the code is barely perfunctory, the art is never there. So this is a good chance for me to improve my Python skills. Also, the age of AI has make _coding_ easy, but this only make _algorithmic thinking_ even more important: we are now, at all time, inundated by a sea of code, and we can only sharpen our swimming skills to avoid being drown.

The way AOC works, we have one new puzzle each day, which has two parts: part 1 is often simpler, and can be tackled with brute force; part 2 would usally need some more structured thinking, and better algorithmic design.

The actual problems to solve are often buried in a big bowl of word salad, about the elves this and the Christmas that. We have to skim away all the elf bumbling to clearly extract the real problems. But alas, such is also the case in life: the problems we have to deal with are seldomly clearly stated, and it's only through clear thinking and meticulous dissection can we find out what are what we should do, what we can do, and how the solution should be assembled together. I took the subject of computer science to be simply _the study and management of complexity_, so problem formulation, along with the solving of it, is a key part of the learning process.

Accompanying each day's input there is also a smaller example input, which is a worked out example to explain the problem, to understand the data, and to demonstrate the expected solution patterns. It's very helpful as a stepstone to solving the real problem, but sometimes it can also be a double edged sword, because there might be patterns or complexities that are present in the real data, but not in the example, so we might be led astray if we rely completely on the example data to construct the solutions. For this reason, although the examples are used in deriving the solutions, they are not present here in the solutions, to avoid confusions.

Revisions:

-   <span class="timestamp-wrapper"><span class="timestamp">[2026-03-19 jeu.] </span></span> updated intro.


## 0. Introduction {#0-dot-introduction}

We use the `aocd` package to interact with Advent of Code programmatically, for which a browser session is needed.
...
Originally I solved each day's puzzle in a separate Jupyter notebook, using the SolveIt platform. When writing the post, I decided to refactor the solutions to each day into three distinct functions, a `parse_day_x` function that get the data into desired format, and two `solve_part_x` functions that solve each part. This is useful for grasping the complete problem solving process, and for clean organization. However this hinders the extraction of programming patterns common to ALL the problems, patterns that might be useful in real programming tasks.

So right now I'm in the process of a third refactoring, extracting the common patterns as separate utility functions, and calling them in the `solve_partx` functions, so that both useful reusable patterns and complete solutions are present. I'm also read Peter Norvig's solutions, trying to pick up skills along the way.


## 1. Rotating dials at the entrance {#1-dot-rotating-dials-at-the-entrance}
...
## 2. Invalid product IDs in the gift shop {#2-dot-invalid-product-ids-in-the-gift-shop}
...
## 3. Maximum joltages of the lobby escalator {#3-dot-maximum-joltages-of-the-lobby-escalator}
...
## 4. Accessible paper rolls in the print shop {#4-dot-accessible-paper-rolls-in-the-print-shop}
...
## 5. Fresh items in the cafeteria {#5-dot-fresh-items-in-the-cafeteria}
...
## 6. Cephalopod math in the trash compactor {#6-dot-cephalopod-math-in-the-trash-compactor}
...
## 7. Splitting beams in the lab Tachyon manifolds {#7-dot-splitting-beams-in-the-lab-tachyon-manifolds}
...
## 8. Connecting junction boxes in the playground {#8-dot-connecting-junction-boxes-in-the-playground}
...
## 9. Rectangular tile patterns in the movie theatre {#9-dot-rectangular-tile-patterns-in-the-movie-theatre}
...
## 10. Combining machine switches in the factory {#10-dot-combining-machine-switches-in-the-factory}
...
## 11. Counting server paths for the reactor {#11-dot-counting-server-paths-for-the-reactor}
...
## 12. Fitting present shapes in the Christmas tree farm {#12-dot-fitting-present-shapes-in-the-christmas-tree-farm}
