...
And make some plots of the raw data. First look at the variable we are trying to model, the `complaints`.

``` {python}
max_complaints = int(pest_data.complaints.max())
complete_range = np.arange(0, max_complaints + 1)

complaint_counts = pest_data['complaints'].value_counts().reindex(complete_range, fill_value=0)
complaint_counts_df = pd.DataFrame({'complaints': complaint_counts.index, 'count': complaint_counts.values})

sns.barplot(data=complaint_counts_df, x='complaints', y='count', color=colors[0]);
```
...
def run_mcmc(model, X, Y, nwarmup=2000, nsample=2000, nchain=2, no_det=True):
    ...
    rng_key = random.PRNGKey(10)
    mcmc = MCMC(kernel, num_warmup=nwarmup, num_samples=nsample, num_chains=nchain, chain_method='vectorized')
...
The term $\text{sq\_foot}$ is called an exposure term. If we log the term, we can put it in $\eta_{n}$:
...
When modeling the data using a Poisson distribution, we saw that the model didn't fit as well to the data as we would like. In particular the model underpredicted low and high numbers of complaints, and overpredicted the medium numbers. This is one indication of over-dispersion, that is, the observed data have more variance than the distribution used to model them. A Poisson model doesn't fit over-dispersed count data very well because the same parameter $\lambda$ controls both the expected counts and the variance of these counts. 

To put the problem in a broader context, this is an inherent limitation of all parametric models. We are modeling **reality** with a probability distribution (a highly abstract mathematical construct) that is controlled by a small number of parameters, and those few parameters will consequently determine some aspects of the model behavious. If these behavious match the attributes of the data that we are trying to model, then we are in luck. If not, we should look for other distributions that can better capture the attributes of the data, or maybe try a mixture of different distributions.

The natural alternative here is the negative binomial model, which augments the Poisson distribution with a **concentration** parameter, $\phi$, as $\text{Neg-Binomial}(y; \lambda, \phi)$, so that we can separate the modeling of mean and variance. Using this parameterisation, the mean and variance are
...
As we can see, the mean is the same as before, but now the variance is always bigger than the mean: this is how we meet the over-dispersion demand. As $\phi$ gets larger, the distribution becomes more concentrated, the term $\lambda^2 / \phi$ approaches zero, the variance of the negative-binomial approaches $\lambda$, and the Negative Binomial gets closer and closer to the Poisson.

Approaching the problem from a different perspective, since our goal is to use a distribution to model the count data, with over-dispersion, another natural approache, if one Poisson distribution with constant $\lambda$ is too limited, is to use a family of Poissons with varying $\lambda$.  And since $\lambda$ is a positive real number, we might consider using a Gamma distribution as its prior, and this leads to a compound Gamma-Poisson distribution. It turns out that this is just another parameterisation of the Negative Binomial distribution.

The Gamma-Poisson distribution implementation in NumPyro is parameterised with a concentration parameter `c` and a rate parameter `r`, but awkwardly have nothing to do with the concentration and rate parameters before. 
...
Because this natively implemented version works better with other NumPyro functionalities, we'll use this distribution to model our data.

### Model

using the mathematical manipulation above, we can write the Gamma Poisson model as:
...
### Bespoke diagnostics: posterior prediction by groups
...
``` {python}
vi_prior_pred = get_prior_pred(vi, vi_X, n=1000)
vi_prior_pred.keys()
```
...
We can see that most of the $\lambda$s are small, but we have a very long tail with extremely large realizations.
...
we have some samples with numerical overflow
...
We won't go on expanding the model, because it looks like our model is already good enough. But good enough for what? Each model building practice should have the specific model, and the specific decisions to make about the model in mind. With observed social science data, we can never build a model good enough to capture all the aspects of the data, or how the data are actually generated. All that we can do, when a problem arises, is to construct a model that solves or approximately solves the problem. Since we didn't touch on the decision making part in this post, we'll stop the model building here because the model outcome data seems to resemble the observed data enough.
