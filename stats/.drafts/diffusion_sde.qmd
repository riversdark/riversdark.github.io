---
title: Diffusion generative model for MNIST
jupyter: python3
---
...
def mnist():
    ...
    with gzip.open(target, "rb") as fh:
        ...
        dataset = jnp.array(array.array("B", fh.read()), dtype=jnp.uint8).reshape(shape)

    data_mean, data_std = jnp.mean(dataset), jnp.std(dataset)
...
class MixerBlock(eqx.Module):
    """
    A MLP-Mixer block that applies mixing operations across patches and channels.
    """
...
class Mixer2d(eqx.Module):
    """
    A 2D Mixer model for image processing tasks.

    This model applies a series of mixing operations on patches of the input image,
    followed by a final convolution to produce the output.
    """
...
num_steps = 50000  # 100000, #1_000_000
lr = 3e-4
batch_size = 256
print_every = 10_000
