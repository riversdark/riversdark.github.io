...
def visualize_graph(G, color, labels=True):
...
The GCNConv layer is defined by the following equation:
...
- $\hat{D}_{ii} = \sum_{j=0} \hat{A}_{ij}$ is its diagonal degree matrix

The adjacency matrix can include values other than 1 to represent edge weights, which can be passed via the optional `edge_weight` tensor.
