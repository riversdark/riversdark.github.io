...
``` {python}
import pyreadr
result = pyreadr.read_r('data/pest_data.RDS')
pest_data = result[None]  
pest_data.columns
```
...
natural alternative to this is the negative binomial model, which augments the Poisson
...
Approaching the problem from a different perspective, since our goal is
...
``` {python}
import pyreadr
result = pyreadr.read_r('data/pest_data_longer_stan_dat.RDS')
pest_more = result[None]  
pest_more.columns
```
